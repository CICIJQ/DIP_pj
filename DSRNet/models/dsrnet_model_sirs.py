...
class DSRNetModel(DSRNetBase):
    ...
    def __init__(self):
        ...
        self.split_intro = False
        self.intro_device = self.device
        self.body_device = self.device
    ...
    def initialize(self, opt):
        ...
        self.configure_inference_split(opt)

        if opt.no_verbose is False:
            self.print_network()

    def configure_inference_split(self, opt):
        split_devices = getattr(opt, "split_intro_devices", "")
        if not split_devices:
            return
        if self.isTrain:
            raise ValueError("--split_intro_devices is only supported for inference")
        if not torch.cuda.is_available():
            raise ValueError("--split_intro_devices requires CUDA")

        parts = [part.strip() for part in split_devices.split(",") if part.strip()]
        if len(parts) != 2:
            raise ValueError("--split_intro_devices must look like '0,1'")

        self.intro_device = torch.device("cuda:%d" % int(parts[0]))
        self.body_device = torch.device("cuda:%d" % int(parts[1]))
        self.vgg = self.vgg.to(self.intro_device)
        self.network.intro = self.network.intro.to(self.intro_device)

        for module in [
            self.network.encoders,
            self.network.decoders,
            self.network.middle_blks,
            self.network.ups,
            self.network.downs,
            self.network.lrm,
            self.network.ending,
        ]:
            module.to(self.body_device)

        self.split_intro = True
        print("[i] split inference: VGG+intro on %s, remaining DSRNet on %s" % (self.intro_device, self.body_device))
    ...
    def forward(self):
        ...
        input_i = self.input
        if self.split_intro:
            input_intro = input_i.to(self.intro_device, non_blocking=True)
            feats = self.vgg(input_intro)
            x, y = self.network.intro(input_intro, feats)
            x = x.to(self.body_device, non_blocking=True)
            y = y.to(self.body_device, non_blocking=True)

            encs = []
            for encoder, down in zip(self.network.encoders, self.network.downs):
                x, y = encoder(x, y)
                encs.append((x, y))
                x, y = down(x, y)

            x, y = self.network.middle_blks(x, y)

            for decoder, up, (enc_x_skip, enc_y_skip) in zip(self.network.decoders, self.network.ups, encs[::-1]):
                x, y = up(x, y)
                x, y = x + enc_x_skip, y + enc_y_skip
                x, y = decoder(x, y)

            output_rr = self.network.lrm(x, y)
            output_t, output_r = self.network.ending(x, y)
