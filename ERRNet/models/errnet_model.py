...
class ERRNetModel(ERRNetBase):
    ...
    def _forward_input(self, input_tensor):
        input_i = input_tensor

        if self.vgg is not None:
            hypercolumn = self.vgg(input_tensor)
        ...
        return self.net_i(input_i)

    def forward(self):
        # without edge
        output_i = self._forward_input(self.input)
    ...
    def forward_tta(self):
        ...
        for dims in (None, (3,), (2,), (2, 3)):
            ...
            output_i = self._forward_input(input_i)
            if dims is not None:
                output_i = torch.flip(output_i, dims=dims)
            outputs.append(output_i)

        output_i = torch.stack(outputs, dim=0).mean(dim=0)
        self.output_i = output_i
...
class ERRNetCascadeModel(ERRNetModel):
    def name(self):
        return 'errnet_cascade'

    def initialize(self, opt):
        opt.inet = 'errnet_cascade'
        super(ERRNetCascadeModel, self).initialize(opt)

        coarse_path = getattr(opt, 'coarse_icnn_path', None)
        if coarse_path and not opt.resume:
            state_dict = _torch_load_compat(coarse_path, map_location=torch.device('cpu'))
            coarse_state = state_dict['icnn'] if 'icnn' in state_dict else state_dict
            missing, unexpected = self.net_i.coarse_net.load_state_dict(coarse_state, strict=False)
            print('[i] initialized cascade coarse net from {}'.format(coarse_path))
            if missing:
                print('[i] missing coarse keys: {}'.format(len(missing)))
            if unexpected:
                print('[i] unexpected coarse keys: {}'.format(len(unexpected)))
    ...
    def backward_G(self):
        ...
        if self.aligned:
            ...
            coarse = getattr(self.net_i, 'last_coarse', None)
            if coarse is not None and getattr(self.opt, 'lambda_coarse', 0) > 0:
                self.loss_coarse_pixel = self.loss_dic['t_pixel'].get_loss(
                    coarse, self.target_t)
    ...
    def get_current_errors(self):
        ret_errors = super(ERRNetCascadeModel, self).get_current_errors()
        if self.loss_coarse_pixel is not None:
            ret_errors['Coarse'] = self.loss_coarse_pixel.item()
        return ret_errors

    def get_current_visuals(self):
        ret_visuals = super(ERRNetCascadeModel, self).get_current_visuals()
        coarse = getattr(self.net_i, 'last_coarse', None)
        if coarse is not None:
            ret_visuals['coarse'] = tensor2im(coarse).astype(np.uint8)
        return ret_visuals


class ERRNetTransformerCascadeModel(ERRNetCascadeModel):
    def name(self):
        return 'errnet_transformer_cascade'

    def initialize(self, opt):
        opt.inet = 'errnet_transformer'
        ERRNetModel.initialize(self, opt)
        ...
        if coarse_path and not opt.resume:
            ...
            coarse_state = state_dict['icnn'] if 'icnn' in state_dict else state_dict
            missing, unexpected = self.net_i.coarse_net.load_state_dict(coarse_state, strict=False)
            print('[i] initialized transformer coarse net from {}'.format(coarse_path))
            if missing:
                print('[i] missing coarse keys: {}'.format(len(missing)))
            if unexpected:
                print('[i] unexpected coarse keys: {}'.format(len(unexpected)))
