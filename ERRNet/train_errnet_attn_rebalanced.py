...
def train_one_epoch(model, dataloader, opt):
    ...
    for i, data in enumerate(dataloader):
        ...
        # rebalanced weights: stronger SSIM, weaker VGG
...
def main():
    ...
    # rebalanced experiment: stronger SSIM, weaker VGG
    opt.lambda_ssim = getattr(opt, 'lambda_ssim', 1.0)
    opt.lambda_grad = getattr(opt, 'lambda_grad', 1.0)
    opt.lambda_l1 = getattr(opt, 'lambda_l1', 1.0)
    opt.lambda_gan = getattr(opt, 'lambda_gan', 0)
    opt.lambda_vgg = getattr(opt, 'lambda_vgg', 0.05)
    ...
    if not opt.no_log:
        ...
        # new dedicated folder for this experiment
        log_name = join(opt.checkpoints_dir, opt.name, 'loss_log_attn_rebalanced.txt')
