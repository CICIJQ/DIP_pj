...
def save_eval_metrics(rows, avg_meters, savedir, dataset_name=None):
    if not rows:
        return

    mkdirs(savedir)
    preferred_keys = ['PSNR', 'SSIM', 'NCC', 'LMSE']
    metric_keys = [key for key in preferred_keys if key in avg_meters.keys()]
    metric_keys.extend(sorted(key for key in avg_meters.keys() if key not in metric_keys))

    csv_path = os.path.join(savedir, 'per_image_metrics.csv')
    with open(csv_path, 'w', newline='') as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=['image'] + metric_keys)
        writer.writeheader()
        for row in rows:
            writer.writerow({key: row.get(key, '') for key in ['image'] + metric_keys})

    summary_path = os.path.join(savedir, 'summary.txt')
    with open(summary_path, 'w') as summary_file:
        if dataset_name is not None:
            summary_file.write('Dataset: %s\n' % dataset_name)
        summary_file.write('Images: %d\n\n' % len(rows))
        summary_file.write('Averages\n')
        for key in metric_keys:
            summary_file.write('%s %.6f\n' % (key, avg_meters[key]))

    print('[i] saved per-image metrics to %s' % csv_path)
